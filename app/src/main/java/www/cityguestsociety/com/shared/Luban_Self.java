...
/**
 * Created by LuoPan on 2017/9/27 10:41.
 */

public class Luban_Self implements Handler.Callback {
    ...
    private Luban_Self(Builder builder) {
                ...
                AsyncTask.SERIAL_EXECUTOR.execute(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            File result = Checker_Self.isNeedCompress(mLeastCompressSize, path) ? new Engine(path, getImageCacheFile(context, Checker_Self.checkSuffix(path))).compress() :
                                    new File(path);
    ...
    private File get(String path, Context context) throws IOException {
        return new Engine(path, getImageCacheFile(context, Checker_Self.checkSuffix(path))).compress();
    }
    ...
    private List<File> get(Context context) throws IOException {
        ...
        while (iterator.hasNext()) {
            ...
            if (Checker_Self.isImage(path)) {
                results.add(new Engine(path, getImageCacheFile(context, Checker_Self.checkSuffix(path))).compress());
            }
            iterator.remove();
        }
    ...
    public static class Builder {
        ...
        private Luban_Self build() {
            return new Luban_Self(this);
        }
