...
/**
 * Created by LuoPan on 2017/5/26 15:10.
 */
public class CustomDatePicker {
    ...
    public CustomDatePicker(Context context, String title, ResultHandler resultHandler, String startDate, String endDate) {
        if (isValidDate(startDate, "yyyy-MM-dd HH:mm") && isValidDate(endDate, "yyyy-MM-dd HH:mm")) {
            ...
            try {
                ...
                changeDay=startCalendar.get(Calendar.DAY_OF_MONTH);
                changeYear=startCalendar.get(Calendar.YEAR);
                changeMonth=startCalendar.get(Calendar.MONTH)+1;
    ...
    private void initView() {
        year_pv = (PickerView) datePickerDialog.findViewById(R.id.year_pv);
        month_pv = (PickerView) datePickerDialog.findViewById(R.id.month_pv);
        day_pv = (PickerView) datePickerDialog.findViewById(R.id.day_pv);
        hour_pv = (PickerView) datePickerDialog.findViewById(R.id.hour_pv);
        mHourLists = new ArrayList<>();
        mHourLists.add("09:00-10:00");
        mHourLists.add("10:00-11:00");
        mHourLists.add("11:00-12:00");
        //        hourLists.add("12:00-13:00");
        mHourLists.add("13:00-14:00");
        mHourLists.add("14:00-15:00");
        mHourLists.add("15:00-16:00");
        mHourLists.add("16:00-17:00");
        mHourLists.add("17:00-18:00");
        Calendar c = Calendar.getInstance();//可以对每个时间域单独修改
        final int systenCurrentYear = c.get(Calendar.YEAR);
        final int systemCurrentMouth = c.get(Calendar.MONTH)+1;

        final int systemCurrentDay = c.get(Calendar.DAY_OF_MONTH);

        int systemCurrentHour = c.get(Calendar.HOUR_OF_DAY);

        if(changeMonth>13){
            changeMonth=1;
        }
        if (systemCurrentDay != changeDay|systenCurrentYear!=changeYear|systemCurrentMouth!=changeMonth) {

        } else {
            if (systemCurrentHour >= 10) {
                mHourLists.remove(0);//移除9-10点
            }
            if (systemCurrentHour >= 11) {
                mHourLists.remove(0);//移除10-11点
            }
            if (systemCurrentHour >= 12) {
                mHourLists.remove(0);//移除11-12点
            }
            if (systemCurrentHour >= 14) {
                mHourLists.remove(0);//移除13-14点
            }
            if (systemCurrentHour >= 15) {
                mHourLists.remove(0);//移除14-15点
            }
            if (systemCurrentHour >= 16) {
                mHourLists.remove(0);//移除15——16点
            }
            if (systemCurrentHour >= 17) {
                mHourLists.remove(0);//移除16-17点
            }
            if(systemCurrentHour>=18){
                mHourLists.remove(0);
                mHourLists.add("休息时间");
            }

        }
        hour_pv.setData(mHourLists);
        ...
        tv_cancle.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                changeDay=systemCurrentDay;
                changeMonth=systemCurrentMouth;
                changeYear=systenCurrentYear;
    ...
    private void addListener() {
        year_pv.setOnSelectListener(new PickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                ...
                changeYear=Integer.parseInt(text);
                monthChange();
                initView();
            }
        });
        ...
        month_pv.setOnSelectListener(new PickerView.onSelectListener() {
            @Override
            public void onSelect(String text) {
                changeMonth=Integer.parseInt(text);
        ...
        day_pv.setOnSelectListener(new PickerView.onSelectListener() {
            ...
            public void onSelect(String text) {
                ...
                changeDay=Integer.parseInt(text);
                hourChange();
                initView();
            }
        });
